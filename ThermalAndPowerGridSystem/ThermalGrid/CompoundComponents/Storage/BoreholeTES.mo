...
model BoreholeTES "Stratified Tank Model...Geometry: Cylinder"
  extends Modelica.Icons.StratifiedTank;
...
/*Uside*/
  ...
  for i in 1:node loop
    density * areaOfNode[i] * cp * der(Tnode[i, 1]) = (+areaOfNode[i] * thermalConductivity * (DoubleDer[i, :] * Tnode[:, 1])) - Qloss[i];
/*(density * Vw * cp * (SingleDer[i, :] * Tnode[:, 1]))*/
  end for;
  Tvariation[:] = Tnode[:, 1];
  annotation(

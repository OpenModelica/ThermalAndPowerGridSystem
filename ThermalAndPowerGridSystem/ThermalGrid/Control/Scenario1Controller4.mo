...
model Scenario1Controller4 " Central Controller for Scenario 2"
  ...
  Modelica.Blocks.Interfaces.BooleanOutput solarOutletPumpControl annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput temperatureTES annotation(
    Placement(visible = true, transformation(origin = {-82, -42}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-52, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
algorithm
  when initial() then
    modeBoiler := false;
    modeSolar := true;
...
//    modeSolar := false;
//  end when;
////  when modeTES and T_SUP < T_DH_LL then  //new
////    modeBoiler := true;
////    modeTES := false;
////  end when;
  when T_SUP < T_DH_L then
    modeSolar := false;
    if temperatureTES>T_DH_L then 
        modeTES := true;
    else
       modeBoiler := true;
    end if;   

...
model BoreholeTESports "Stratified Tank Model...Geometry: Cylinder"
  ...
  parameter PerUnit Uside = 0.3 "Heat transfer coefficient";
      ThermalAndPowerGridSystem.ThermalGrid.Interfaces.WaterPort inlet annotation(
    Placement(visible = true, transformation(origin = {-98, 58}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-110, 62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ThermalAndPowerGridSystem.ThermalGrid.Interfaces.WaterPort outlet annotation(
    Placement(visible = true, transformation(origin = {108, -2}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -54}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
Vw = mfr/density;
inlet.massFlowRate + outlet.massFlowRate = 0;
inlet.pressure + density*Modelica.Constants.g_n*heightTank= outlet.pressure;
inlet.specificEnthalpy = inStream(inlet.specificEnthalpy);
outlet.specificEnthalpy = FluidWater.specificEnthalpy_pTX(outlet.pressure, Tvariation[node],{1});
inletTemperature =  FluidWater.temperature_phX(inlet.pressure, inlet.specificEnthalpy,{1});
